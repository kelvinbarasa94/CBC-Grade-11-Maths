/* Intersecting chords (cf. Figure 10.3.12).
 * Chords AB (blue) and CD (red) meet at P, with AP*PB = CP*PD.
 * P is placed at the origin; A,B on one direction, C,D on the other, on opposite
 * sides of P. By the converse of the intersecting-chords theorem the four points
 * are automatically concyclic, so the circumcircle of any three passes through all.
 * In STACK the first line becomes  var AP={#AP#}, PB={#PB#}, CP={#CP#}, PD={#PD#};
 */
var AP = 6, PB = 4, CP = 8, PD = 3;

var rad = Math.PI / 180;
var aAB = 20 * rad;      // direction of chord AB (A in -dir, B in +dir)
var aCD = 75 * rad;      // direction of chord CD (C in -dir, D in +dir)
...
/* circumcentre of A, B, C (plain maths, before the board exists) */
...
/* segment-length labels, nudged perpendicular to each chord */
var nAB = [-uy, ux], nCD = [-vy, vx], off = 0.55;
...
lbl(Ap, [0, 0], nAB,  1, AP + ' cm');     // AP
lbl([0, 0], Bp, nAB, -1, PB + ' cm');     // PB
lbl(Cp, [0, 0], nCD,  1, CP + ' cm');     // CP
lbl([0, 0], Dp, nCD,  1, 'x cm');         // PD (unknown)
